...
requires_gpu = unittest.skipUnless(TEST_CUDA or TEST_XPU, "Requires CUDA or XPU")
...
def forward(self, pred_1, x_1):
    ...
    def test_switch_gpu(self):
        ...
        x = torch.randn(4, device=device_type)
        index = torch.tensor(1, device=device_type)
    ...
    def test_switch_autograd_gpu(self):
        ...
        for i, fn in enumerate(branches):
            x = torch.randn(4, requires_grad=True, device=device_type)
            result = switch(torch.tensor(i, device=device_type), branches, (x,))
            self.assertEqual(result, fn(x))
...
class AssociativeScanTests(TestCase):
    ...
    def test_associative_scan_pointwise_mixed_device_lowering_error(self):
        ...
        a = torch.randn(8, 4, device=device_type)
...
class TestAutoFunctionalizeControlFlow(TestCase):
    ...
    def test_cond_auto_functionalize_input_mutation(self, device, dynamic):
        ...
        if not TEST_WITH_CROSSREF and not dynamic and device == device_type:
            self.assertExpectedInline(
